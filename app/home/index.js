    ...
    const deleteHabit = async () => {
        try {
            ...
            if (response.status == 200) {
                // Assuming the response.data is an array of habits
                setHabit(response.data);
            }
        } catch (error) {
            console.log("error", error);
        ...
        <>
            <ScrollView style={{ flex: 1, backgroundColor: "white", padding: 10 }}>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10, marginVertical: 8 }}>
                    <Pressable onPress={() => setOption("Today")} style={{ backgroundColor: option == "Today" ? "#E0FFFF" : "transparent", paddingHorizontal: 10, paddingVertical: 8, borderRadius: 25 }}>
                ...
                {option == "Today" && (filteredHabits?.length > 0 ? (
                    <View>
                        {filteredHabits?.map((item, index) => (
                            <Pressable onLongPress={() => handleLongPress(item._id)} style={{ marginVertical: 10, backgroundColor: item?.color, padding: 12, borderRadius: 24 }}>
                                <Text style={{ textAlign: "center", fontWeight: "600", color: "white" }}>{item?.title}</Text>
                            </Pressable>
                        ))}
                    </View>
                ) : (
                    <View style={{ marginTop: 150, justifyContent: "center", alignItems: "center", marginBottom: "auto" }}>
                        ...
                        <Pressable onPress={() => router.push("/home/create")} style={{ backgroundColor: "#0071c5", marginTop: 20, paddingHorizontal: 20, paddingVertical: 10, marginLeft: "auto", marginRight: "auto" }}>
                            <Text>Create</Text>
                        </Pressable>
                    </View>
                ))}
                        ...
                        {habits?.map((habit, index) => (
                            <Pressable style={{ marginVertical: 10, backgroundColor: habit.color, padding: 15, borderRadius: 24 }}>
                                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                                    ...
                                    <Text style={{ fontSize: 15, fontWeight: "500", color: "white" }}>{habit.repeatMode}</Text>
                                </View>
                                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-evenly", marginVertical: 10 }}>
                                    {days?.map((day, item) => {
                                        ...
                                        return (
                                            <Pressable>
                                                <Text style={{ color: day === currentDay ? "red" : "white" }}>{day}</Text>
                                                {isCompleted ? (
            ...
            >
                <ModalContent style={{ width: "100%", height: 270 }}>
                    <View style={{ marginVertical: 10 }}>
                        <Text>Options</Text>
                        <Pressable onPress={handleCompletion} style={{ flexDirection: "row", alignItems: "center", gap: 12, marginTop: 10 }}>
                        ...
                        <Pressable onPress={deleteHabit} style={{ flexDirection: "row", alignItems: "center", gap: 12, marginTop: 10 }}>
